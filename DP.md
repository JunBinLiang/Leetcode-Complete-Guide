...
#### 分析： 
 ...
 - 通过观察，我们每次移除的数字都是从左边开始，我们可以用一个 **_``l``_** _(index)_  去表示当前还剩下那些数字。剩下的数字是 **``A[l : len(A)-1]``** 。 
...
**_接下来是我们的 ``BottomUp``了_**
 ...
 - 首先我们的状态是以**``dp[_l_][player]``** 表示的，**``_l_``** 表示当前所剩下的石头的最左边(别忘了我们只能从左边开始取)，**``player``** 表示当前是谁
 - 我们只需要将上面递归的转换一下就可以了
