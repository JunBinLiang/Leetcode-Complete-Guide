...
<h4 align="center">:heavy_minus_sign: :heavy_minus_sign: :heavy_minus_sign: :heavy_minus_sign: :heavy_minus_sign: 有了简单的了解之后让我们来看看我们的石子游戏吧! :heavy_minus_sign: :heavy_minus_sign: :heavy_minus_sign: :heavy_minus_sign: :heavy_minus_sign:  <h4>
...
**_:heavy_exclamation_mark: 下面并不是完结的代码。我们还需要对每个人采用的 ``决策 (博弈)`` 进行分析_**
...
:small_orange_diamond: 以上是 ``TopDown`` 的 DP 方式，非常的好实现，并且从时间空间复杂度来说与 ``BottomUp`` 并没有什么区别。但是它的问题是因为用的是递归，所以会对 ``runtime stack`` 有很大的要求。我们再来看``BottomUp`` 是怎么写的，我们可以透过 ``TopDown`` 已经写好了的关系来进行一点简单的改变就行了。
